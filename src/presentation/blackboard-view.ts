...
export class BlackboardView extends TextFileView {
  ...
  private zoomHud: HTMLElement | null = null;
  ...
  private enterEditMode(): void {
    ...
    let fileObjects: DiagramObject[] = [];
    let filePage: Background | undefined;
    let fileInkProfile: InkProfile | undefined;
    if (this.fileData) {
      try {
        ...
        fileObjects = result.file.objects ?? [];
        filePage = result.file.page;
        fileInkProfile = result.file.inkProfile;
      } catch {
    ...
    this.engine.loadObjects?.(fileObjects);
    this.engine.setPage?.(filePage);
    this.engine.setInkProfile?.(fileInkProfile);
    if (!this.isEmbedded) this.buildZoomHud();
  ...
  private async attachToStore(): Promise<void> {
    ...
    try {
      ...
      handle.subscribe(() => {
        ...
        this.engine.loadObjects?.(this.handle.getFile().objects ?? []);
        this.engine.setPage?.(this.handle.getFile().page);
        this.engine.setInkProfile?.(this.handle.getFile().inkProfile);
        this.engine.staticDirty = true;
  ...
  private buildFile(): BlackboardFile {
    ...
    const objects = this.engine!.strokeManager.objects;
    const page = this.engine!.getPage?.();
    const inkProfile = this.engine!.getInkProfile?.();
    return {
      version: 3,
  ...
  private downloadCurrentSvg(selectionOnly = false): void {
  ...
  private buildZoomHud(): void {
    if (!this.drawingContainer || this.zoomHud) return;
    this.zoomHud = this.drawingContainer.createDiv({ cls: 'blackboard-zoom-hud' });
    this.zoomLabel = this.zoomHud.createDiv({ cls: 'blackboard-zoom-label' });
    this.updateZoomHud();
  }

  private updateZoomHud(): void {
    if (!this.zoomLabel || !this.engine) return;
    const view = this.engine.getViewTransform?.();
    if (!view) return;
    this.zoomLabel.textContent = `${Math.round(view.scale * 100)}%`;
  }
  ...
  setViewData(data: string, clear: boolean): void {
    ...
    if (this.engine) {
      ...
      try {
        ...
        this.engine.loadObjects?.(result.file.objects ?? []);
        this.engine.setPage?.(result.file.page);
        this.engine.setInkProfile?.(result.file.inkProfile);
  ...
  private setupDocumentListeners(): void {
    ...
    const isInsideDrawing = (e: PointerEvent): boolean => {
      ...
      return dc.contains(target) && !target.closest('.blackboard-global-toolbar');
    };
    ...
    const handlePoint = (e: PointerEvent): void => {
      ...
      if (tool === 'eraser') {
        engine.setEraserCursor?.(localX, localY);
        this.eraseAt(localX, localY);
        return;
      }
      if (tool === 'line' || tool === 'arrow' || tool === 'rectangle' || tool === 'ellipse') {
        engine.updateShape(localX, localY);
      } else if (tool === 'selection') {
        engine.updateSelection(localX, localY);
      } else {
    ...
    const onDocPointerDown = (e: PointerEvent) => {
      ...
      if (!drawInput) return;
      // Apple Pencil wins over an already-resting palm. The old code rejected the Pencil
      // whenever any touch pointer existed, which made a palm on the canvas block writing.
      if (!this.isEmbedded && this.navPointers.size > 0) {
      ...
      if (tool === 'selection') {
        engine.beginSelection(localX, localY);
      } else if (tool === 'line' || tool === 'arrow' || tool === 'rectangle' || tool === 'ellipse') {
        engine.beginShape(tool, localX, localY);
      } else if (tool !== 'eraser') {
    ...
    const onDocPointerUp = (e: PointerEvent) => {
      ...
      const tool = engine.toolManager.activeTool;
      if (tool === 'selection') {
        if (engine.endSelection()) this.persist();
      } else if (tool === 'line' || tool === 'arrow' || tool === 'rectangle' || tool === 'ellipse') {
        engine.endShape();
        this.persist();
      } else if (tool !== 'eraser') {
    ...
    const onDocPointerCancel = (e: PointerEvent) => {
      if (!this.isEmbedded && this.navPointers.delete(e.pointerId)) {
        this.pinchPrev = null;
        e.stopPropagation();
        return;
      }
    ...
    activeDocument.addEventListener('pointercancel', onDocPointerCancel, true);
    ...
    this.docListeners.push({ type: 'pointercancel', fn: onDocPointerCancel, capture: true });

    const onDeleteSelection = (e: KeyboardEvent) => {
      if ((e.key !== 'Delete' && e.key !== 'Backspace') || engine.toolManager.activeTool !== 'selection') return;
    ...
    activeDocument.addEventListener('keydown', onDeleteSelection, true);
    this.docListeners.push({ type: 'keydown', fn: onDeleteSelection, capture: true });
