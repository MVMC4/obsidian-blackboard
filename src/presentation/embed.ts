...
import type { BlackboardFile, DiagramObject, PluginSettings, Stroke } from '../domain/entities';
...
export async function mountBlackboardEmbed(repo: IDrawingRepository, embedEl: HTMLElement, filePath: string, settings: PluginSettings, surfaceManager?: SurfaceManager, toolManager?: ToolManager, store?: DocumentStore): Promise<() => void> {
  ...
  engine.loadObjects?.(refFile().objects ?? []);
  engine.setPage?.(refFile().page);
  engine.setInkProfile?.(refFile().inkProfile);
  ...
  if (handle) {
    handle.subscribe(() => {
      ...
      engine.loadObjects?.(handle.getFile().objects ?? []);
      engine.setPage?.(handle.getFile().page);
      engine.setInkProfile?.(handle.getFile().inkProfile);
  ...
  async function saveDrawing(): Promise<void> {
    try {
      ...
      const objects = engine.strokeManager.objects;
      const page = engine.getPage?.();
      const inkProfile = engine.getInkProfile?.();
  ...
  const surface = engineSurface(engine, () => { void saveDrawing(); }, (selectionOnly) => {
  ...
  function isInsideDrawing(e: PointerEvent): boolean {
    ...
    return drawingContainer.contains(target) && !target.closest('.blackboard-global-toolbar');
  }
  ...
  function handlePoint(e: PointerEvent): void {
    ...
    if (tool === 'eraser') { engine.setEraserCursor?.(localX, localY); eraseAt(localX, localY); return; }
  ...
  const onDocPointerUp = (e: PointerEvent) => {
    ...
    drawingContainer.setCssStyles({ touchAction: 'pan-x pan-y' });
    e.stopPropagation();
    const tool = engine.toolManager.activeTool;
    if (tool === 'selection') {
  ...
  docListeners.push({ type: 'pointercancel', fn: onDocPointerCancel, capture: true });
  ...
  docListeners.push({ type: 'keydown', fn: onDeleteSelection, capture: true });
