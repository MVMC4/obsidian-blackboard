...
const TOOLS: Array<{ name: ToolName; icon: IconName; label: string }> = [
  ...
  { name: 'selection', icon: 'select', label: 'Select and move' },
  { name: 'line', icon: 'line', label: 'Line' },
  { name: 'arrow', icon: 'arrow', label: 'Arrow' },
  { name: 'rectangle', icon: 'rectangle', label: 'Rectangle' },
  { name: 'ellipse', icon: 'ellipse', label: 'Ellipse' },
];
...
export class GlobalToolbar {
  ...
  private pageBtn!: HTMLButtonElement;
  private pagePopover!: HTMLElement;
  private pageOptions!: HTMLElement;
  ...
  private stylePopover!: HTMLElement;
  private styleOptions!: HTMLElement;
  ...
  private build(): void {
    ...
    this.surfaceControls.push(deleteBtn);
    this.deleteBtn = deleteBtn;
    this.root.appendChild(deleteBtn);
    this.root.appendChild(this.separator());
  ...
  private buildPagePopover(): void {
    this.pagePopover = this.host.createDiv({ cls: 'blackboard-gt-popover blackboard-gt-page-popover' });
    this.pagePopover.setCssStyles({ display: 'none' });
    this.pagePopover.addEventListener('pointerdown', (e) => e.stopPropagation());

    const label = this.pagePopover.createDiv({ cls: 'blackboard-gt-popover-label' });
    label.textContent = 'Paper style';
    this.pageOptions = this.pagePopover.createDiv({ cls: 'blackboard-gt-page-options' });
    const options: Array<{ type: Background['type']; label: string }> = [
      { type: 'blank', label: 'Blank' },
      { type: 'dot-grid', label: 'Dots' },
      { type: 'line-grid', label: 'Ruled' },
      { type: 'square-grid', label: 'Grid' },
    ];
    for (const option of options) {
      const button = createEl('button');
      button.className = 'blackboard-gt-page-option';
      button.dataset.pageType = option.type;
      button.textContent = option.label;
      button.addEventListener('pointerup', (e) => {
        e.stopPropagation();
        this.applyPageType(option.type);
      });
    ...
    for (const preset of [16, 24, 32, 48]) {
      const button = createEl('button');
      button.className = 'blackboard-gt-page-preset';
      button.textContent = String(preset);
      button.dataset.gridSize = String(preset);
      button.addEventListener('pointerup', (e) => {
        e.stopPropagation();
        this.setPageGridSize(preset);
      });
      this.pagePresetRow.appendChild(button);
    }
    this.pageSizeNumber = createEl('input');
    this.pageSizeNumber.type = 'number';
    this.pageSizeNumber.min = '8';
    this.pageSizeNumber.max = '200';
    this.pageSizeNumber.step = '1';
    this.pageSizeNumber.className = 'blackboard-gt-page-number';
    this.pageSizeNumber.setAttribute('aria-label', 'Grid spacing');
    this.pageSizeNumber.addEventListener('change', () => this.setPageGridSize(Number(this.pageSizeNumber.value)));
    this.pagePopover.appendChild(this.pageSizeNumber);
    this.pageSizeRange = createEl('input');
    this.pageSizeRange.type = 'range';
    this.pageSizeRange.min = '12';
    this.pageSizeRange.max = '80';
    this.pageSizeRange.step = '4';
    this.pageSizeRange.className = 'blackboard-gt-page-size';
    this.pageSizeRange.addEventListener('input', () => this.setPageGridSize(Number(this.pageSizeRange.value)));
    this.pagePopover.appendChild(this.pageSizeRange);
  }

  private setPageGridSize(value: number): void {
  ...
  private buildInkPopover(): void {
    this.inkPopover = this.host.createDiv({ cls: 'blackboard-gt-popover blackboard-gt-ink-popover' });
    this.inkPopover.setCssStyles({ display: 'none' });
    this.inkPopover.addEventListener('pointerdown', (e) => e.stopPropagation());
    const label = this.inkPopover.createDiv({ cls: 'blackboard-gt-popover-label' });
    label.textContent = 'Ink fidelity';
    this.inkOptions = this.inkPopover.createDiv({ cls: 'blackboard-gt-option-row' });
    const options: Array<{ mode: InkProfileMode; label: string }> = [
      { mode: 'raw', label: 'Raw' },
      { mode: 'natural', label: 'Natural' },
      { mode: 'pressure', label: 'Pressure' },
    ];
    for (const option of options) {
      const button = createEl('button');
      button.className = 'blackboard-gt-option';
      button.dataset.inkMode = option.mode;
      button.textContent = option.label;
      button.addEventListener('pointerup', (e) => {
        e.stopPropagation();
        this.surface?.setInkProfile?.({ ...INK_PROFILES[option.mode] });
        this.syncInkControls();
        this.closePopovers();
      });
      this.inkOptions.appendChild(button);
    }
  }

  private buildStylePopover(): void {
    this.stylePopover = this.host.createDiv({ cls: 'blackboard-gt-popover blackboard-gt-style-popover' });
    this.stylePopover.setCssStyles({ display: 'none' });
    this.stylePopover.addEventListener('pointerdown', (e) => e.stopPropagation());
    const label = this.stylePopover.createDiv({ cls: 'blackboard-gt-popover-label' });
    label.textContent = 'Shape line style';
    this.styleOptions = this.stylePopover.createDiv({ cls: 'blackboard-gt-option-row' });
    const options: Array<{ style: 'solid' | 'dashed' | 'dotted'; label: string }> = [
  ...
  private buildExportPopover(): void {
    this.exportPopover = this.host.createDiv({ cls: 'blackboard-gt-popover blackboard-gt-export-popover' });
  ...
  private currentPage(): Background {
    return this.surface?.page ?? {
      type: 'blank', color: 'transparent', grid: false, gridSize: 24, gridColor: '#9aa4b2',
    };
  }

  private applyPageType(type: Background['type']): void {
    if (!this.surface?.setPage) return;
    const current = this.currentPage();
    this.surface.setPage({
      ...current,
      type,
      grid: type !== 'blank',
    });
  ...
  /** Toolbar: always top-centre of the active view. */
  private placeToolbar(): void {
    ...
    this.root.style.top = `${Math.max(b.top, b.top + 12)}px`;
  }
  ...
  private sync(): void {
    ...
    this.deleteBtn.disabled = this.surface.hasSelection !== true;
  ...
  private syncPageControls(): void {
    ...
    for (const button of Array.from(this.pagePresetRow.querySelectorAll<HTMLElement>('[data-grid-size]'))) {
      button.classList.toggle('active', Number(button.dataset.gridSize) === page.gridSize);
    }
  ...
  private syncInkControls(): void {
    if (!this.surface) return;
    const mode = this.surface.inkProfile?.mode ?? 'raw';
    this.inkBtn.classList.toggle('active', mode !== 'raw');
    for (const button of Array.from(this.inkOptions.querySelectorAll<HTMLElement>('[data-ink-mode]'))) {
      button.classList.toggle('active', button.dataset.inkMode === mode);
    }
  }

  private syncStyleControls(): void {
    if (!this.surface) return;
    const style = this.surface.selectedLineStyle;
    this.styleBtn.classList.toggle('active', style !== null && style !== undefined);
    for (const button of Array.from(this.styleOptions.querySelectorAll<HTMLElement>('[data-line-style]'))) {
      button.classList.toggle('active', button.dataset.lineStyle === style);
    }
  }
  ...
  private togglePopover(which: 'color' | 'size' | 'page' | 'ink' | 'style' | 'export'): void {
    const target = which === 'color' ? this.colorPopover : which === 'size' ? this.sizePopover : which === 'page' ? this.pagePopover : which === 'ink' ? this.inkPopover : which === 'style' ? this.stylePopover : this.exportPopover;
    const popovers = [this.colorPopover, this.sizePopover, this.pagePopover, this.inkPopover, this.stylePopover, this.exportPopover];
    const other = popovers.filter((p) => p !== target);
    for (const p of other) p.setCssStyles({ display: 'none' });
    ...
    if (open) {
      ...
      if (which === 'page') this.syncPageControls();
      if (which === 'ink') this.syncInkControls();
      if (which === 'style') this.syncStyleControls();
      if (which === 'export') this.syncExportControls();
  ...
  destroy(): void {
    ...
    this.pagePopover.remove();
    this.inkPopover.remove();
    this.stylePopover.remove();
