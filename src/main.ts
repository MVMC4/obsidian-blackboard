...
export default class BlackboardPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.registerEvent(this.app.workspace.on('layout-change', () => { ensureToolbar(); this.globalToolbar?.reposition(); }));
    try {
    ...
    this.addCommand({
      id: 'export-active-drawing-svg',
      name: 'Export active drawing as SVG',
      callback: async () => {
        const file = this.app.workspace.getActiveFile();
        if (!file || file.extension !== FILE_EXTENSION) return;
        try {
          await this.exportSvgUseCase.execute(file.path, this.settings.svgExportPath);
        } catch {
          // Export is best-effort; repository errors should not disable the plugin command.
        }
      },
    });
  ...
  private routeToolbar(): void {
    ...
    // A surface may have become active before the toolbar was constructed during Obsidian
    // startup. Refresh after routing so the toolbar receives the current surface immediately.
    this.surfaceManager.refresh();
  }
