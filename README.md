...
- **Pen, highlighter, eraser** — Raw, natural, and pressure ink profiles with a visible eraser radius.
- **Diagramming tools** — Lines, arrows, rectangles, ellipses, dashed/dotted styles, accurate mixed selection, batch delete, recoloring, and shape resizing.
- **Visible export** — Export the complete drawing or only the selected elements as SVG from the toolbar.
- **Paper presets** — Blank, dots, ruled, and square paper with repeatable 16/24/32/48 spacing presets plus a numeric spacing field.
...
- **Shared floating toolbar** — One toolbar follows the active drawing from the top of the view. Tool, color, and brush size are global across every drawing on the page; it collapses to a corner pill.
...
Use the toolbar's **Export** button to export the complete drawing or the current selection as SVG. You can also enable **Auto-export SVG** in settings to write a `.svg` next to each drawing whenever it is saved (renames and deletes are kept in sync). Set **SVG export path** to collect exports in one folder.
...
| Export active drawing as SVG | Export the active standalone `.blackboard` drawing |
