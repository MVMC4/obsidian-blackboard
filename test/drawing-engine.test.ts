...
describe('DrawingEngine — diagram selection', () => {
  it('selects, moves, and deletes a diagram object', () => {
    ...
    engine.updateSelection(70, 65);
    expect(engine.endSelection()).toBe(true);
    expect(engine.strokeManager.objects[0].x).toBe(50);
    expect(engine.strokeManager.objects[0].y).toBe(45);

    expect(engine.deleteSelection()).toBe(true);
    expect(engine.strokeManager.objects).toHaveLength(0);
    engine.strokeManager.undo();
    expect(engine.strokeManager.objects[0].id).toBe('box-1');
  });

  it('lasso-selects objects and clears selection on an empty tap', () => {
    const engine = new DrawingEngine(createMockContainer(), 800, 600);
    engine.loadObjects([
      { id: 'a', kind: 'rectangle', x: 10, y: 10, width: 30, height: 30, color: '#fff', size: 2, opacity: 1, fill: 'transparent', fillOpacity: 0, timestamp: 1 },
      { id: 'b', kind: 'rectangle', x: 100, y: 100, width: 30, height: 30, color: '#fff', size: 2, opacity: 1, fill: 'transparent', fillOpacity: 0, timestamp: 2 },
    ]);
    engine.beginSelection(0, 0);
    engine.updateSelection(60, 60);
    engine.endSelection();
    expect(engine.getSelection().objectIds).toEqual(['a']);

    engine.beginSelection(400, 400);
    engine.endSelection();
    expect(engine.hasSelection()).toBe(false);
  });

  it('prioritizes a stroke inside a box over the enclosing box', () => {
    const engine = new DrawingEngine(createMockContainer(), 800, 600);
    engine.loadObjects([{
      id: 'box', kind: 'rectangle', x: 10, y: 10, width: 100, height: 80,
      color: '#fff', size: 2, opacity: 1, fill: 'transparent', fillOpacity: 0, timestamp: 1,
    }]);
    engine.loadStrokes([{
      id: 'text', tool: 'pen', color: '#fff', size: 2, opacity: 1,
      points: [[45, 45, 0.5], [55, 45, 0.5]], hasPressure: false, timestamp: 2,
    }]);

    engine.beginSelection(50, 45);
    engine.endSelection();

    expect(engine.getSelection()).toEqual({ strokeIds: ['text'], objectIds: [] });
  });

  it('resizes a selected shape from its bottom-right handle and supports undo', () => {
    const engine = new DrawingEngine(createMockContainer(), 800, 600);
    engine.loadObjects([{
    ...
    engine.endSelection();
    engine.beginSelection(80, 60);
    engine.updateSelection(120, 100);
    expect(engine.endSelection()).toBe(true);
    expect(engine.strokeManager.objects[0].width).toBe(100);
    expect(engine.strokeManager.objects[0].height).toBe(80);
    engine.strokeManager.undo();
