...
describe('eraseAtPoint', () => {
  ...
  it('deletes diagram objects as well as freehand strokes', () => {
    strokeManager.addObject({
    ...
    const result = eraseAtPoint(strokeManager, 20, 20, 15);

    expect(result).toBe(true);
    expect(strokeManager.objects).toHaveLength(0);
  });

  it('keeps a rectangle when erasing inside its perimeter', () => {
    strokeManager.addObject({
      id: 'box-2', kind: 'rectangle', x: 20, y: 20, width: 80, height: 50,
      color: '#fff', size: 2, opacity: 1, fill: 'transparent', fillOpacity: 0, timestamp: 1,
    });

    const result = eraseAtPoint(strokeManager, 60, 45, 15);

    expect(result).toBe(false);
    expect(strokeManager.objects).toHaveLength(1);
  });
});
