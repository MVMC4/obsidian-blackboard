...
describe('getStroke options follow the selected ink profile', () => {
  beforeEach(() => {
    mockGetStroke.mockClear();
  });

  it('committed stroke render uses raw, unsmoothed defaults', () => {
    ...
    expect(opts.smoothing).toBe(0);
    expect(opts.streamline).toBe(0);
  ...
  it('SVG export uses the same raw defaults', () => {
